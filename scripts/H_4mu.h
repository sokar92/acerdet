...
// Thu Jul  2 11:03:44 2015 by ROOT version 5.34/30
// from TTree ACDTree/ACDTree
// found on file: new_a-det/pythia.pp.to.H.to.ZZ.4mu.conf.root
//////////////////////////////////////////////////////////
...
#ifdef H_4mu_cxx
H_4mu::H_4mu(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("../conf/pythia.pp.to.H.to.ZZ.4mu.conf.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("../conf/pythia.pp.to.H.to.ZZ.4mu.conf.root");
      }
...
Int_t H_4mu::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t H_4mu::LoadTree(Long64_t entry)
{
...
void H_4mu::Show(Long64_t entry)
{
...
Int_t H_4mu::Cut(Long64_t entry)
{
