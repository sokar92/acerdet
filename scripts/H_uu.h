...
// Fri Jul  3 16:33:23 2015 by ROOT version 5.34/30
// from TTree ACDTree/ACDTree
// found on file: new_a-det/pythia.pp.to.H.to.uu.conf.root
//////////////////////////////////////////////////////////
...
#ifdef H_uu_cxx
H_uu::H_uu(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("../conf/pythia.pp.to.H.to.uu.conf.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("../conf/pythia.pp.to.H.to.uu.conf.root");
      }
...
Int_t H_uu::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t H_uu::LoadTree(Long64_t entry)
{
