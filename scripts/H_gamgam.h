...
// Wed Jul  1 14:16:44 2015 by ROOT version 5.34/30
// from TTree ACDTree/ACDTree
// found on file: new_a-det/acerdet-master/conf/pythia.pp.to.H.to.gamgam.conf.root
//////////////////////////////////////////////////////////
...
#ifdef H_gamgam_cxx
H_gamgam::H_gamgam(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("../conf/pythia.pp.to.H.to.gamgam.conf.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("../conf/pythia.pp.to.H.to.gamgam.conf.root");
      }
...
Int_t H_gamgam::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t H_gamgam::LoadTree(Long64_t entry)
{
...
void H_gamgam::Show(Long64_t entry)
{
...
Int_t H_gamgam::Cut(Long64_t entry)
{
